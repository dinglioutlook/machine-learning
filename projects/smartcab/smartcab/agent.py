...
class LearningAgent(Agent):
    ...
    def reset(self, destination=None, testing=False):
        ...
        if testing:
            ...
            #self.epsilon = 0.5 * math.exp(-self.iteration / 5)
            #self.epsilon = self.epsilon - 0.05
            #self.epsilon = self.alpha ** self.iteration # FF
            # self.epsilon = 1 / (self.iteration ** 2) # FF
            #self.epsilon = math.exp(-self.alpha * self.iteration)
            self.epsilon = math.pow(0.994, self.iteration)
            self.iteration += 1
    ...
    def choose_action(self, state):
        ...
        if not self.learning:
            ...
            if self.epsilon > random.random():
                ...
                action = random.choice([act for act in self.valid_actions if self.get_maxQ(state) == self.Q[state][act]])
